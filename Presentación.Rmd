...
labs(x = "Distancia", y = "Semivarianza", title = "Variogramas de los modelos espaciales")
...
```{r , echo= TRUE}
ggplot(Z_vgm_null, aes(x = dist, y = gamma)) + geom_point() + geom_hline(yintercept = 0.05698552, lty = 2) + geom_vline(xintercept = 80.19058, lty = 2) + theme_bw() + labs(x = "Distancia", y = "Semivarinza", title = "Modelo Nulo") 

ggplot(Z_vgm_spatial, aes(x = dist, y = gamma)) + geom_point() + geom_hline(yintercept = 0.05458574, lty = 2) + geom_vline(xintercept = 75.58536, lty = 2) + theme_bw() + labs(x = "Distancia", y = "Semivarinza", title = "Modelo Espacial")

ggplot(Z_vgm_interaccion1, aes(x = dist, y = gamma)) + geom_point() + geom_hline(yintercept = 0.03680731, lty = 2) + geom_vline(xintercept = 62.5634, lty = 2) + theme_bw() + labs(x = "Distancia", y = "Semivarinza", title = "Modelo Interacción 1")

ggplot(Z_vgm_interaccion2, aes(x = dist, y = gamma)) + geom_point() + geom_hline(yintercept = 4.196248e-05 + 3.370992e-02, lty = 2) + geom_vline(xintercept = 61.50199, lty = 2) + theme_bw() + labs(x = "Distancia", y = "Semivarinza", title = "Modelo Interacción 2")

ggplot(Z_vgm_mixto, aes(x = dist, y = gamma)) + geom_point() + geom_hline(yintercept = 0.03413205, lty = 2) + geom_vline(xintercept = 61.85772, lty = 2) + theme_bw() + labs(x = "Distancia", y = "Semivarinza", title = "Modelo Mixto 1")

ggplot(Z_vgm_mixto2, aes(x = dist, y = gamma)) + geom_point() + geom_hline(yintercept = 0.04281398, lty = 2) + geom_vline(xintercept = 69.32882, lty = 2) + theme_bw() + labs(x = "Distancia", y = "Semivarinza", title = "Modelo Mixto 2")

ggplot(Z_vgm_mixto3, aes(x = dist, y = gamma)) + geom_point() + geom_hline(yintercept = 0.03673725, lty = 2) + geom_vline(xintercept = 63.00708, lty = 2) + theme_bw() + labs(x = "Distancia", y = "Semivarinza", title = "Modelo Mixto 3")
```
...
```{r ,echo= TRUE}
Null_CV <- krige.cv(log(DvrsddA) ~ 1, Diver_2, model = Abn_fit_null, nfold = 7) %>% st_as_sf() %>% mutate(Modelo = "Nulo")

Spat_CV <- krige.cv(log(DvrsddA) ~ coords.x1 + coords.x2, Diver_2, model = Abn_fit_Spat, nfold = 7) %>% st_as_sf() %>% mutate(Modelo = "Espacial")

Interaccion1_CV <- krige.cv(log(DvrsddA) ~ ModificacionHumana + I(ModificacionHumana^2) + TempMedia + I(TempMedia^2) + RangoTempDiario + I(RangoTempDiario^2) + PrecAnual + I(PrecAnual^2) + PrecAnual + I(PrecAnual^2) + EstacionalidadPrec + I(EstacionalidadPrec^2), Diver_2, model = Abn_fit_Interaccion1, nfold = 7) %>% st_as_sf() %>% mutate(Modelo = "Interacción 1")

Interaccion2_CV <- krige.cv(log(DvrsddA) ~ TempMedia*(RangoTempDiario*ModificacionHumana)*EstacionalidadPrec +  I((ModificacionHumana)^2) + I((TempMedia)^2) + RangoTempDiario + I((RangoTempDiario)^2) + EstacionalidadPrec + I((EstacionalidadPrec)^2), Diver_2, model = Abn_fit_Interaccion2, nfold = 7) %>% st_as_sf() %>% mutate(Modelo = "Interacción 2")

Mixto_CV <- krige.cv(log(DvrsddA) ~ (coords.x1 + coords.x2) + TempMedia*(RangoTempDiario*ModificacionHumana)*EstacionalidadPrec, Diver_2, model = Abn_fit_Mixto, nfold = 7) %>% st_as_sf() %>% mutate(Modelo = "Mixto 1")

Mixto2_CV <- krige.cv(log(DvrsddA) ~ (coords.x1 + coords.x2) + ModificacionHumana + I(ModificacionHumana^2), Diver_2, model = Abn_fit_Mixto2, nfold = 7) %>% st_as_sf() %>% mutate(Modelo = "Mixto 2")

Mixto3_CV <- krige.cv(log(DvrsddA) ~ ModificacionHumana*TempMedia + I((ModificacionHumana)^2) + I((TempMedia)^2) + RangoTempDiario + I((RangoTempDiario)^2) + EstacionalidadPrec + I((EstacionalidadPrec)^2), Diver_2, model = Abn_fit_Mixto2, nfold = 7) %>% st_as_sf() %>% mutate(Modelo = "Mixto 3")
...
ggplot() + geom_sf(data = Pred_CV, aes(color = residual)) + facet_wrap(~fold) + scale_color_viridis_c()+ theme(axis.text.x = element_text(angle =90, vjust = 0.5, hjust=1))
